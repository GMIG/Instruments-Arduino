...
#define MAX_SENSE_RESULT MAX_COMMAND_RESULT + 20
...
int readTransport(LEDControl* senses, int NUM_OF_SENSES, char* result){

        if (Serial.available() > 0) {

            char* cmdStr = Serial.readBytesUntil('\n',commandBuffer,MAX_COMMAND_SIZE);
...
*/
            ...
            for (int i = 0; i < NUM_OF_SENSES;i++) {
                LEDControl* s = senses[i];
                if(strcmp(s->name(),senseString) == 0){
